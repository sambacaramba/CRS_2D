function CRScomplex_occurences = CRS2D_complex(fitted_line,optimalpath,savepath, fname, image, pixelsize, mov_window_size,angle_range,imagesave,artefactmask)
...
if imagesave
savepath_mov = [savepath '\movie'];

 A = exist(savepath_mov);
    if A == 0
    mkdir(savepath_mov);
    else 
    end  
...
for col = 1 + window_size : rows - window_size -1
...
if ~isnan(middleindex)&&middleindex>window_size&&middleindex<rows-window_size&&length(fitted_line)>middleindex+half_window
...
CRS2D_complex_moviesave(image, fitted_line, optimalpath,mov_window_size,pixelsize,midindex,window_complex,window_fitted,savepath_mov, fname, RunN,p_complex,p_fitted,angle_deg)
end 
end
...
%     plot([datanew(i,2) datanew(i+1,2)], [datanew(i,1) datanew(i+1,1)], 'Color', color, 'LineWidth', 1);
% end
% 
